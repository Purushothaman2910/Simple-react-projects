...
function Profile() {
    ...
    let { name,theme , toggleTheme} = useTheme()
  return (
    <>
    <div className = {`${(theme === THEMES.light) ? 'bg-white' : 'bg-black'} w-full h-screen flex flex-col gap-10 items-center justify-center`}>
        <p className={`text-3xl  shadow-lg ${(theme === THEMES.light)?'shadow-black ':'bg-black shadow-white text-white'} font-mono font-semibold p-4 rounded`}> Hi {user} {name} </p>
        <button 
